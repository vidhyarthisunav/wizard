$('#moredogs').click(function(){
    ...
    $('<div class="question"><div class="sub-question"> <div class="question-divider"> <span><span class="nameofpet"></span> is...</span> </div> <div class="btn--group"> <button onclick="setClass(this)" class="btn btn--pill " role="button" type="button"> <span>Very fussy</span> </button> <button onclick="setClass(this)" class="btn btn--pill " role="button" type="button"> <span>Fussy</span> </button> <button onclick="setClass(this)" class="btn btn--pill " role="button" type="button"> <span>Not fussy</span> </button> <button onclick="setClass(this)" class="btn btn--pill " role="button" type="button"> <span>A foodie</span> </button> </div> </div>').insertBefore('#addfussy');
    
    $('<div class="sub-question"><div class="question-divider"><span><span class="nameofpet"/> eats...</span></div><div class="btn--group"><button onclick="setClass(this)" class="btn btn--pill btn--multi" role="button" type="button"><span>Dry</span></button><button onclick="setClass(this)" class="btn btn--pill btn--multi" role="button" type="button"><span>Wet</span></button><button onclick="setClass(this)" class="btn btn--pill btn--multi" role="button" type="button"><span>Raw</span></button><button onclick="setClass(this)" class="btn btn--pill btn--multi" role="button" type="button"><span>Home-cooked</span></button></div></div>').insertBefore('#foodtype');
    ...
    $('<div class="question"> <div class="sub-question"><div class="question-divider"> <span><span class="nameofpet"></span> is allergic to...</span> </div> <div class="btn--group"> <button onclick="setClass(this)" class="btn btn--pill btn--pill--selected" type="button" id="nothing"> <span>Nothing</span> </button> <button onclick="setClass(this)" class="btn btn--pill btn--multi " role="button" type="button" id="chicken"> <span>Chicken</span> </button> <button onclick="setClass(this)" class="btn btn--pill btn--multi " role="button" type="button" id="cheese"> <span>Cottage Cheese</span> </button> </div> <div> <div class="checkbox-wrapper"> <input type="checkbox" aria-labelledby="unknown-allergies-label-0" role="button" id="unknown-allergies-input-0"> <label class="checkbox-label">I&#39;m not sure yet</label> </div> </div> </div>').insertBefore('#addallergy');

    $('<div class="sub-question"> <div class="question-divider"> <span><span class="nameofpet"></span> has...</span> </div> <div class="health-issues"> <div class="btn--group"> <button onclick="setClass(this)" class="btn btn--pill" role="button" type="button" id="nohealthbutton"> <span>No health issues</span> </button> <button onclick="setClass(this)" class="btn btn--pill " role="button" type="button" id="somehealthbutton"> <span>Some health issues</span> </button> <div role="combobox" aria-owns="typeahead-list-box0" aria-haspopup="listbox" aria-expanded="false" class="typeahead-combobox" style="display:none;" id="healitemsuesfield"> <input type="text" class="typeahead-input" aria-controls="typeahead-list-box0" aria-autocomplete="typeahead-listbox" aria-label="Search for your dog&#39;s health issues" placeholder="health issues"> </div> </div> </div> </div>').insertBefore('#addissues');
    ...
    $('<div class="sub-question"> <div class="question-divider"> <span><span class="nameofpet"></span>...</span> </div> <div class="snacks"> <div class="snacks__example-row"> <div role="button" tabindex="0" class="snacks__example-row__item btn btn--radio"> <img class="default-image" alt="an outline of a dog treat" src="./images/no-treats.svg"> <img class="selected-image" alt="an outline of a dog treat" src="./images/no-treats-selected.svg"> </div> <div role="button" tabindex="0" class="snacks__example-row__item btn btn--radio snacks__example-row__item--selected btn--radio--selected"> <img class="default-image" alt="a few dog treats" src="./images/some-treats.svg"> <img class="selected-image" alt="a few dog treats" src="./images/some-treats-selected.svg"> </div> <div role="button" tabindex="0" class="snacks__example-row__item btn btn--radio"> <img class="default-image" alt="a big bunch of dog treats" src="./images/lots-of-treats.svg"> <img class="selected-image" alt="a big bunch of dog treats" src="./images/lots-of-treats-selected.svg"> </div> </div> <div class="snacks__description snacks__description--second" id="partyframe"> <p id="partyhead">Has some</p> <p id="partycontent">1-3 times a day</p> </div> </div> </div>').insertBefore('#addlast');
      ...
      $('.snacks__example-row__item').click(function(){
        $(item).siblings().removeClass("snacks__example-row__item--selected btn--radio--selected");
        $(item).addClass("snacks__example-row__item--selected btn--radio--selected");
        if($(item).is(':first-child')){
          $(item).parent().siblings().removeClass();
          $(item).parent().siblings().addClass("snacks__description snacks__description--first");
  
        }
        if($(item).is(':nth-child(2)')){
          $(item).parent().siblings().removeClass();
          $(item).parent().siblings().addClass("snacks__description snacks__description--second");
        }
        if($(item).is(':nth-child(3)')){
          $(item).parent().siblings().removeClass();
          $(item).parent().siblings().addClass("snacks__description snacks__description--third");
        }
      })
      
      $('.body-type__example-row__item').click(function(){
        $(item).siblings().removeClass("body-type__example-row__item--selected btn--radio--selected");
        $(item).addClass("body-type__example-row__item--selected btn--radio--selected");
        if($(item).is(':first-child')){
          $(item).parent().parent().siblings().removeClass();
          $(item).parent().parent().siblings().addClass("body-type__description body-type__description--first");
        }
        if($(item).is(':nth-child(2)')){
          $(item).parent().parent().siblings().removeClass();
          $(item).parent().parent().siblings().addClass("body-type__description body-type__description--second");
        }
        if($(item).is(':nth-child(3)')){
          $(item).parent().parent().siblings().removeClass();
          $(item).parent().parent().siblings().addClass("body-type__description body-type__description--third");
          item.parentElement.parentElement.siblings
        }
      })
      $('.body-type__example-row__item').click(function(){
        $(item).siblings().removeClass("body-type__example-row__item--selected btn--radio--selected");
      ...
      $('.activity-level__example-row__item').click(function(){
        $(item).siblings().removeClass("activity-level__example-row__item--selected btn--radio--selected");
        $(item).addClass("activity-level__example-row__item--selected btn--radio--selected");
        if($(item).is(':first-child')){
          $(item).parent().siblings().removeClass();
          $(item).parent().siblings().addClass("activity-level__description activity-level__description--second");
        }
        if($(item).is(':nth-child(2)')){
          $(item).parent().siblings().removeClass();
          $(item).parent().siblings().addClass("activity-level__description activity-level__description--fourth");
        }
      })
  })